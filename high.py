...
def app ():
    st.title('Максимальная цена сделки за день')
    ticker_symbol = 'AAPL'
    data = yf.Ticker(ticker_symbol)
    hist = data.history(period="5y")
    st.subheader("Последние 5 дней")
    st.write(hist['High'].tail())
    st.subheader("Последние 5 лет")
    fig = px.line(hist, y='High', title="Максимальная цена котировки Apple (AAPL)")
    st.plotly_chart(fig)
    ...
    file_name='apple_quotes_high.csv', # Имя файла, которое будет у скачанного файла
    mime='text/csv')